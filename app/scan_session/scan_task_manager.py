...
class ScanTaskManager(object):
    def __init__(self):
        self.task_list_file = os.path.join(basedir, "scan_task", "task_list.json")
    ...
    def load_task_info(self, task_name):
        ...
        with open(os.path.join(task_path, "task_info.json"), 'r', encoding='utf-8') as rf:
            # 载入任务信息
    ...
    def save_task(self, task_name):
        ...
        # 保存扫描结果
        with open(os.path.join(task_path, task_info.scan_result_file), 'w', encoding='utf-8') as wf:
    ...
    def load_all_task_info(self):
        ...
        if len(self.scan_task_list):
            for task in self.scan_task_list:
                ...
                task_info_dict["Timestamp"] = time.strftime("%Y/%m/%d %H:%M:%S",
